/**
 * registrar-formacion — Guarda una participación de un curso de "Formación"
 * (creado por el colegio, multi-tenant) y crea su certificado verificable.
 *
 * POST /api/registrar-formacion
 * Body JSON: { tenant_id, colegio, curso_id, curso_titulo, modulo_id, modulo_titulo,
 *              nombre, email, puntaje, total_preguntas, codigo, consentimiento, honeypot }
 *
...
exports.handler = async (event) => {
  ...
  if (typeof codigo !== 'string' || !/^FOR-[A-Z0-9]{4}-[A-Z0-9]{4}$/.test(codigo)) {
  ...
  const participacion = {
    ...
    curso_id: String(curso_id || '').trim(),
    curso_titulo: String(curso_titulo || '').trim(),
  ...
  try {
    ...
    const r1 = await fetch(`${base}/formacion_participaciones?key=${apiKey}`, {
    ...
    const r2 = await fetch(`${base}/formacion_certificados?documentId=${encodeURIComponent(codigo)}&key=${apiKey}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    // 3) Copia por colegio (Fase 4: regla formacion_rendidos). Tolerante: si aún
    //    no está la regla, el 403 no debe romper el registro principal.
    if (tid) {
      try {
        const registro = {
          ...
          estudianteClave: '', estudianteNombre: '', estudianteRun: ''
        };
        await fetch(`${base}/tenants/${encodeURIComponent(tid)}/formacion_rendidos?documentId=${encodeURIComponent(codigo)}&key=${apiKey}`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(toFirestoreDoc(registro)),
        });
      } catch (_) { /* Fase 4 aún no habilitada: ignorar */ }
    }
